...
class Flake8Error:
    ...
    def from_line(cls, line):
        ...
        filename, *rest = prefix.split(":")
        if len(rest) < 2:
            raise ValueError("Invalid flake8 error line: [%s]" % line)
        line_number = rest[0]
        col_number = rest[1]
    ...
    *,
    ...
    for error in errors:
        if not show_line_numbers:
            lines.append("- %s" % error.problem)
        else:
            lines.append(
                "- %s:%s %s" % (error.line_number, error.col_number, error.problem)
            )
    return "\n".join(lines)
